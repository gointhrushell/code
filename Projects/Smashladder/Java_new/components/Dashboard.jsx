...
import {PreferredGames} from "../models/PreferredGames";
import {TabChanger, TabList} from "./TabList";
...
ladderHistory.setDashboard(Dashboard);
...
Dashboard.ladderPopup = function(content, title, options){
    ...
    var modalPopupCallbackData = {
        ...
        changeTitleHtml: function(newTitle){
        ...
        showLoader: function(){
            ...
            let applyLoader = ()=>{
                ...
                modal.findCache('.modal-body').prepend(this.loading);
            };
            for(let i = 0; i < 2; i++)
            {
                setTimeout(()=>{
...
Dashboard.challenges = new Challenges();
        ...
        !Dashboard.recentMatchSearchers.hasClass('hard_focus'))
    {
        ...
        currentList.find('.recent_match_searcher').tsort({
            sortFunction: (a,b)=>{
                a = a.e;
                b = b.e;
                let aMatch = a.data('match');
                let bMatch = b.data('match');
...
Dashboard.updatePreferredGames = function(newPreferredGames){
    ...
    function rerunList(name){ //Hack to not get items blacklisted...
        var list = LadderInfo.retrieveReference(name);
        if(list.items)
        {
            for(var i in list.items){
                ...
                LadderInfo.forceRemove(name, i, true);
                var newItem = {};
                newItem[i] = item.data;
                LadderInfo.parseChanges(name, newItem);
            }
        }
    }

    Dashboard.gameFilters.findCache('.dynamic_preferred_games_holder').find('.preferred_game_filter').add(Dashboard.gameFilters.find('.game_settings_selection'))
        .tsort({data:'order_by',order:'asc'});
    ...
    let filters = Dashboard.gameFilters.findCache('.dynamic_preferred_games_holder').find('.preferred_game_filter');
    let onFilters = Dashboard.getNumberOfOnFilteredGames();
...
if(allTabs.length)
{
    ...
    Dashboard.tabList = TabList.attachTabEvents(allTabs);
}

$(function(){
    TabList.attachTabEvents();
});
