...
import {declickables} from "components/Declickables";
...
ChatActions.updateChatMessage = function($element,message,loadingAllMessages)
{
    ...
    else
    {
        ...
        $element.find('.time').text(DateFormat.hourMinutes(message.time))
            .attr('title',DateFormat.full(message.time))
...
//		$element.find('input[name=message_id]').val(message.id);
    ...
    if(message.edited)
    {
        ...
        $element.find('.edited').addClass('active').attr('title', DateFormat.fullNoSeconds(message.edited));
    }
    else
    {
...
ChatActions.attachUniversalChatActions = function(element){
    element.on('click','.send_chat_button', function(e){
        ...
        if(!ChatActions.lastAutocompleteCacheClear ||
            Date.now() - ChatActions.lastAutocompleteCacheClear > 180 * 1000)
        {
        ...
        if(e.which == Dashboard.keyCodes.ENTER)
        {
            e.preventDefault();
            ChatActions.sendChat($(this));
        }
        if(e.which == Dashboard.keyCodes.UP_ARROW)
        {
            let input = $(this);
            if(input.val() === '')
            {
                e.preventDefault();
                let chatContainer = input.data('chatContainer');
                let myMessages = chatContainer.find('.chat_message.is_me:not(.my_temp_message)');
                let lastMessage = myMessages.last();
                if(lastMessage.is(myMessages.filter('.editing')))
                {
                }
                else
                {
...
ChatActions.endEditOnMessage = function(chatMessage, input){
    ...
    let message = chatMessage.find('.message');

    message
        .removeAttr('contentEditable','')
        .html(chatMessage.data('originalValue'));
    if(!Dashboard.isTiny())
    {
        input.focus();
    }
...
ChatActions.startEditOnMessage = function(chatMessage, input){
    ...
    chatMessage.on('keydown', function(e){
       ...
       {
           e.preventDefault();
           let text = $.trim(chatMessage.find('.message').text());
           chatMessage.data('lastSentValue', text);
           let data = {};
           data.message_id = chatMessage.data('message_id');
           data.message = text;
           ChatActions.endEditOnMessage(chatMessage, input);
           Request.send(data, 'send_chat').then(function(){
...
ChatActions.updateUserlist = function(userlist, force = false){
    if(!userlist){
        userlist = ChatActions.getActiveChatContainerUserlist();
    }
    if(userlist && userlist.length && Dashboard.userlistIsVisible())
    {
        setTimeout(function(){
            if(userlist.data('userMap'))
            {
...
ChatActions.repositionChatTabs = function(location){
    let chatGroups = ChatActions.objectCache.chatGroups;
...
ChatActions.sendChatState = function(chatInput){
    ...
    {
        let matchContainer = chatInput.closest('.current_match_container');
...
ChatActions.reportToMods = function(title,description,extraData){
    ...
    let popup = Dashboard.ladderPopup(null, 'Report to Mods').showLoader();
    let popupContent;
    $.get(href, data, function(response){
        let content = $($.trim(response.html));
        let heading = content.find('.report_heading');
        heading.remove();
